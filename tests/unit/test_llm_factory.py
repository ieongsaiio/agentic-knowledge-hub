...
class TestLLMFactory:
    ...
    def test_settings_loads_extra_chat_configs(self, tmp_path):
        """Settings should preserve provider-specific chat configs."""
        config_file = tmp_path / "settings.yaml"
        config_file.write_text(
            create_test_config(
                provider="fake",
                **{
                    "llm.extra_chat_configs": {
                        "top_p": 0.8,
        ...
        assert settings.llm.extra_chat_configs == {
            "top_p": 0.8,
            "reasoning": {"enabled": True},
        }
