...
class TestCollectionInfo:
    ...
    def test_basic_creation(self) -> None:
        ...
        assert info.document_count is None
    ...
    def test_to_dict_with_counts(self) -> None:
        """Test to_dict with document and chunk counts."""
        info = CollectionInfo(name="test", document_count=2, chunk_count=50)
        result = info.to_dict()
...
class TestListCollectionsMethod:
    ...
    def test_list_collections_with_stats(
        ...
        mock_coll2.get.return_value = {
            "ids": ["c3"],
            "metadatas": [{"source_ref": "doc_3"}],
            "documents": ["three"],
        }
        ...
        assert result[1].document_count == 1
        assert result[1].chunk_count == 50
    ...
    def test_list_collections_without_stats(
        ...
        assert result[0].document_count is None
        assert result[0].chunk_count is None
    ...
    def test_list_collections_count_error_graceful(
        ...
        # Should still return collection, but with no statistics
        assert len(result) == 1
...
class TestFormatResponse:
    ...
    def test_format_single_collection(
        ...
        collections = [
            CollectionInfo(name="docs", document_count=2, chunk_count=50)
        ]
        result = tool_with_config.format_response(collections)

        assert "## Available Collections (1 total)" in result
        assert "1. **docs** - 2 documents, 50 chunks" in result
    ...
    def test_format_multiple_collections(
        ...
        assert "1. **knowledge_hub** - 3 documents, 150 chunks" in result
        assert "2. **documents** - 2 documents, 75 chunks" in result
        assert "3. **test_collection** - 0 documents, 0 chunks" in result

    def test_format_singular_counts(
        self,
        tool_with_config: ListCollectionsTool,
    ) -> None:
        collections = [
            CollectionInfo(name="single", document_count=1, chunk_count=1)
        ]

        result = tool_with_config.format_response(collections)

        assert "1. **single** - 1 document, 1 chunk" in result
    ...
    def test_format_with_metadata(
        ...
        assert "3 documents, 30 chunks" in result
    ...
    def test_format_without_counts(
        self,
        tool_with_config: ListCollectionsTool
    ) -> None:
    ...
    def test_format_json(
        ...
        result = tool_with_config.format_json(collections)

        assert '"collection_count": 1' in result
        assert '"document_count": 2' in result
        assert '"chunk_count": 10' in result
        assert "hnsw:space" not in result
...
class TestExecuteMethod:
    ...
    async def test_execute_success(
        ...
        assert "Collections (JSON)" in result.content[1].text
