...
def test_explicit_alias_storage_id_is_preserved(upserter_with_mock_store):
    ...
    assert upserter._generate_chunk_id(chunk) == storage_id
...
def test_upsert_single_chunk(upserter_with_mock_store, sample_chunk, sample_vector):
    ...
    assert "text" not in records[0]["metadata"]
...
def test_upsert_multiple_chunks(upserter_with_mock_store):
    ...
    for i, record in enumerate(records):
        ...
        assert record["document"] == f"Chunk {i}"
        assert "text" not in record["metadata"]
...
def test_upsert_preserves_metadata(upserter_with_mock_store, sample_chunk, sample_vector):
    ...
    # Verify text is stored once as the document, not duplicated in metadata.
    assert records[0]["document"] == sample_chunk.text
