...
MULTI_PAGE_PDF = FIXTURES_DIR / "blogger_intro.pdf"
...
class TestPdfLoaderInitialization:
    ...
    def test_custom_initialization(self):
        ...
        loader = PdfLoader(extract_images=False, image_storage_dir="custom/path")
...
class TestPdfLoaderHelperMethods:
    ...
    def test_pages_from_form_feeds_requires_page_count_match(self):
        pages = PdfLoader._pages_from_form_feeds(
            "page one\fpage two\f",
            page_count=2,
        )
        assert pages == ["page one", "page two"]
        assert (
            PdfLoader._pages_from_form_feeds(
                "page one\npage two",
                page_count=2,
            )
            is None
    ...
    def test_combine_page_texts_records_half_open_ranges(self):
        text, spans = PdfLoader._combine_page_texts(["alpha", "beta"])

        assert text == "alpha\n\nbeta"
        assert spans == [
            {"page": 1, "start_offset": 0, "end_offset": 5},
            {"page": 2, "start_offset": 7, "end_offset": 11},
        ]
        assert text[spans[0]["start_offset"] : spans[0]["end_offset"]] == "alpha"
        assert text[spans[1]["start_offset"] : spans[1]["end_offset"]] == "beta"
...
class TestPdfConversionCore:
    ...
    def test_convert_simple_pdf_to_text(self):
        ...
        assert len(doc.metadata["page_spans"]) == 1

    def test_multi_page_pdf_records_page_spans(self):
        """Every physical PDF page gets an ordered character range."""
        if not MULTI_PAGE_PDF.exists():
            pytest.skip(f"Test fixture not found: {MULTI_PAGE_PDF}")

        loader = PdfLoader(extract_images=False)
        doc = loader.load(MULTI_PAGE_PDF)

        assert doc.metadata["page_count"] == 5
        spans = doc.metadata["page_spans"]
        assert [span["page"] for span in spans] == [1, 2, 3, 4, 5]
        assert len(spans) == doc.metadata["page_count"]
        assert all(span["start_offset"] <= span["end_offset"] for span in spans)
        assert all(
            spans[index]["end_offset"] <= spans[index + 1]["start_offset"]
