...
class BM25Indexer:
    ...
    def _save(self, collection: str) -> None:
        ...
        try:
            ...
            self._replace_with_retry(temp_path, index_path)
    ...
    def _replace_with_retry(self, source: Path, destination: Path) -> None:
        """Atomically replace a BM25 index, retrying transient Windows locks."""
        for attempt in range(self.SAVE_REPLACE_ATTEMPTS):
            try:
                os.replace(source, destination)
                return
            except PermissionError:
                if attempt + 1 >= self.SAVE_REPLACE_ATTEMPTS:
                    raise
                time.sleep(
                    self.SAVE_RETRY_DELAY_SECONDS * (attempt + 1)
                )

    def _unlink_with_retry(self, path: Path) -> None:
        """Remove a temporary BM25 file, tolerating transient Windows locks."""
        for attempt in range(self.SAVE_REPLACE_ATTEMPTS):
            try:
