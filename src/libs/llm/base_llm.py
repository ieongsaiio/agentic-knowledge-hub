...
class BaseLLM(ABC):
    ...
    def _merge_chat_configs(self, overrides: Dict[str, Any]) -> Dict[str, Any]:
        """Merge provider default chat configs with per-call overrides.

        Provider implementations may set ``self.extra_chat_configs`` from
        settings. Runtime keyword arguments passed to ``chat()`` take
        precedence over settings-level defaults.
        """
        merged = dict(getattr(self, "extra_chat_configs", {}) or {})
