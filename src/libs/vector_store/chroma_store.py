    ...
    def clear(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Keep the collection object itself stable. Deleting and recreating
            # a Chroma collection can leave other same-process clients holding a
            # stale internal collection id, which then breaks later upserts.
            existing = collection.get(include=[])
            ids = [str(record_id) for record_id in existing.get("ids", [])]
            batch_size = 5000
    ...
    def _sanitize_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in metadata.items():
            if isinstance(value, (str, int, float, bool)):
                ...
                # Skip None values
                continue
            elif key in {"images", "image_captions"} and isinstance(
                value, (dict, list, tuple)
            ):
                # Preserve multimodal structures in a portable format. Chroma
                # metadata values must be scalar, so decode this JSON on read.
