...
def _lazy_import_cross_encoder_api_reranker():
    """Lazy import to avoid circular dependencies."""
    from src.libs.reranker.cross_encoder_api_reranker import CrossEncoderAPIReranker
...
class RerankerFactory:
    ...
    def create(cls, settings: Settings, **override_kwargs: Any) -> BaseReranker:
        ...
        # Lazy register API-based Cross-Encoder reranker if not already registered
        if "cross_encoder_api" not in cls._PROVIDERS:
            CrossEncoderAPIReranker = _lazy_import_cross_encoder_api_reranker()
            cls.register_provider("cross_encoder_api", CrossEncoderAPIReranker)
